...
export function canManageEmployees(
  ...
  return hasPermission(permissions, 'employees.create') || hasRole(roles, EMPLOYEE_MANAGEMENT_ROLES);
}
...
export function canAccessClients(
  ...
  return hasPermission(permissions, 'customers.read') || hasRole(roles, CLIENT_MANAGEMENT_ROLES);
}